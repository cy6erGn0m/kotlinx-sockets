...
class HeaderEntry(val nameStart: Int, val nameLength: Int, val nameHash: Int, val valueStart: Int, val valueLength: Int) {
    ...
    fun name(array: ByteArray): String = _name ?:
            knownHeaderNames[nameHash]?.firstOrNull { equalsIgnoreCase(array, nameStart, nameLength, it) }?.also { _name = it } ?:
            array.stringOf(nameStart, nameLength).also { _name = it }

    fun value(array: ByteArray): String = _value ?: array.stringOf(valueStart, valueLength).also { _value = it }

    companion object {
        val knownHeaderNames = """
            A-IM
            Accept
            Accept-Additions
            Accept-Charset
            ...
            X-Forwarded-Proto
            X-Forwarded-For
        """.trimIndent().lines().map { it.trim() }.filter { it.isNotEmpty() }.distinctBy { it.toLowerCase() }.groupBy({ it.hashCodeLowerCase() }, { it.intern() })
    }
}
